...
#align(center)[#strong[Closout Report Video Script]]
...
#align(center)[(Estimated Time assumes 120 - 150 words per minute)]
#v(10pt)
= Introduction
...
\ For the successful execution of these technical solutions, we received a total funding of ₳238,374. To date, we have received ₳105,000, with ₳133,374 remaining to be distributed upon the completion of all milestones.
We conducted rigorous testing for all three data structures to ensure their robustness and efficiency.

Each data structure was subjected to unit tests, with results integrated into a CI/CD pipeline.
...
All documentation is well-organized, well-structured, and accessible to developers of varying skill levels and can be found in our #git-link.
...
- *Implementation of Advanced Data Structures:* Our team has pioneered the development and implementation of advanced data structures (Merkle Trees, Tries, and Linked Lists) in Aiken and Plutarch. These structures enhance the scalability of DApps on Cardano by allowing for more efficient use of the limited 16kb transaction size and the single UTXO model.
...
Linked List: We created a Linked List that leverages the EUTXO model to significantly enhance scalability and throughput in smart contracts. By linking multiple UTXOs together through a series of minting policies and validators, it improves the user experience by allowing concurrent interactions with smart contracts. Our documentation provides detailed tutorials, scenarios, and code examples demonstrating the practical applications of Linked Lists in smart contracts.
...
#align(center)[(Total Estimated Time: 8 minutes)]
